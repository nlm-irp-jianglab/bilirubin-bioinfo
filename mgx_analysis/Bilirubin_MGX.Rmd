...
3) classifying each sample as having bilirubin reductase and gus as present or absent based on a 5 CPM threshold. 

```{r}
# Read in overall data file
project_data <- read.csv('./bilirubin_mgx_data.csv', row.names = 1)

# Calculate CPM values for bilR and GUS genes. Remove samples with less than 1 
# million reads
project_data$bilr_tot <- (project_data$bilR/project_data$total_reads)*1000000
project_data$gus_tot <- (project_data$gus/project_data$total_reads)*1000000
...
project_data$gus_presence[project_data$gus_tot <= cpm_threshold] <- 'absent'
project_data$gus_presence[project_data$gus_tot > cpm_threshold] <- 'present'
...
healthy_ibd <- ggplot(bilr_disease, aes(x=host_disease, y=absent_frac*100), ) + 
  ...
  geom_bracket(xmin=c('CD', 'UC'), xmax=c('Healthy', 'Healthy'), label=c('2.2e-16', '2.2e-16'), y.position = c(38, 36), size=0.2, label.size=2.5, vjust=0) + 
...
health_infant <- ggplot(healthy_infant, aes(x=group, y=absent_frac*100), ) + 
  ...
  geom_bracket(xmin=c('Infant (< 1 month)'), xmax=c('Adult'), label=c('2.2e-16'), y.position = c(77), size=0.2, label.size=2.5, vjust=0) + 
